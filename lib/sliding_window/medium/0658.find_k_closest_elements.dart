  ...
  List<int> findClosestElements(List<int> arr, int k, int x) {
    ...
    for (int windowRight = 0; windowRight < arr.length; windowRight++) {
      if (windowRight - windowLeft + 1 > k) {
        final left = arr[windowLeft];
        final right = arr[windowRight];

        final leftDiff = (left - x).abs();
        final rightDiff = (right - x).abs();

        if (leftDiff < rightDiff || (leftDiff == rightDiff && left < right)) {
          return arr.sublist(windowLeft, windowRight);
        }
        windowLeft++;
      }
    }

    return arr.sublist(windowLeft);
  }
}
