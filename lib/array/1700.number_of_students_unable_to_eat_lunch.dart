...
class Solution {
  int countStudents(List<int> students, List<int> sandwiches) {
    ...
    while (sandwiches.isNotEmpty) {
      final currentSandwich = sandwiches.first;
      final currentStudent = students.first;

      if (currentSandwich == currentStudent) {
        sandwiches.removeAt(0);
        students.removeAt(0);
        countOfHungry = 0;
      } else {
        final rem = students.removeAt(0);
        students.add(rem);
        countOfHungry += 1;
        if (countOfHungry == students.length) return countOfHungry;
      }
    }
