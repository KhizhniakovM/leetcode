  ...
  List<String> fullJustify(List<String> words, int maxWidth) {
    ...
    for (int i = 0; i < words.length; i++) {
      if (length + words[i].length + line.length > maxWidth) {
        final extra = maxWidth - length;
        final lineLength = line.length - 1 == 0 ? 1 : line.length - 1;

        final spaces = extra ~/ lineLength;
        int remainder = extra % lineLength;
    ...
    result.add('$lastLine${' ' * (maxWidth - lastLine.length)}');
