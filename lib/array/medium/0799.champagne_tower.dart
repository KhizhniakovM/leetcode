...
class Solution {
  double champagneTower(int poured, int queryRow, int queryGlass) {
    // Начинаем с первого ряда. Заливаем всю шампань в первый бокал
    List<double> prevRow = [poured.toDouble()];

    // Идем по всем последующим рядам до целевого
    for (int row = 1; row < queryRow + 1; row++) {
      // Создаем пустой следующий ряд
      final currentRow = List<double>.generate(row + 1, (_) => 0);
      // Заполняем каждый бокал последующего ряда из бокалов предыдущего ряда - 1
      for (int i = 0; i < prevRow.length; i++) {
        //  То, что не вошло в текущий бокал и льется дальше
        final extra = prevRow[i] - 1;
        if (extra > 0) {
    ...
    return prevRow[queryGlass] > 1 ? 1 : prevRow[queryGlass];
  }
}
