...
class Solution {
  ...
  int calPoints(List<String> operations) {
    ...
    for (final operation in operations) {
      switch (operation) {
        case '+':
          final updated = stack[stack.length - 1] + stack[stack.length - 2];
          stack.add(updated);
        case 'D':
          final updated = stack.last * 2;
          stack.add(updated);
        case 'C':
          stack.removeLast();
        default:
          final n = int.tryParse(operation);
          stack.add(n!);
      }
    }

    return stack.fold<int>(0, (prev, val) => prev + val);
  }
}
