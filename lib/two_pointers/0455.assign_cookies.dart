...
class Solution {
  ...
  int findContentChildren(List<int> g, List<int> s) {
    ...
    for (int coockieIdx = 0; coockieIdx < s.length; coockieIdx++) {
      if (childIdx >= g.length) return childIdx;
      final currentChild = g[childIdx];
      final currentCookie = s[coockieIdx];

      if (currentCookie >= currentChild) {
        childIdx += 1;
      }
    }
